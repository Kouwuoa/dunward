//! Model hierarchies, megabuffer geometry uploads, and fullscreen presentation quads.
//!
...
impl FullscreenQuad {
    ...
    pub fn resize_to_swapchain(
        ...
        let size = swc_ctx.swapchain.swapchain_image_extent;
        ...
        self.quad_model
            .write_vertex_buffer(&vertices_merged, vertex_megabuffer)?;
...
impl Model {
    pub fn new(
        ...
        if !all_meshes_valid {
            return Err(eyre!(
                "All meshes must have either no indices or all indices"
            ));
        }
